# api-conditions-catalog Specification

## Purpose

Global, non-campaign-scoped conditions catalog (`slug`, `name`, `defaultSeverity`, `polarity`, `description?`) readable by any authenticated user, with admin-only batched add/update/rename/delete operations mirroring the existing global-variable schema and skills-catalog conventions.
