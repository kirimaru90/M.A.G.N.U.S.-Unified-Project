# emulator-personal-terminal Specification

## Purpose
A logged-in-only terminal-list entry that opens a character picker and plays the server-generated per-character terminal. It reuses the existing terminal container and playback engine — no new `index.html` element and no new authoring surface — so playback, headers, typing, sound, and back/disconnect all work unchanged.
