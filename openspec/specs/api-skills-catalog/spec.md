# api-skills-catalog Specification

## Purpose

Global, non-campaign-scoped skills catalog (`slug`, `name`, `description?`) readable by any authenticated user, with admin-only batched add/update/rename/delete operations and a one-time default-skills seed on first startup.
