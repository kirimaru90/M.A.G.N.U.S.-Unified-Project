...
Structured request/response logging for the API: a single access line for successful requests and a single enriched error line for failures, with distinct log levels, validation and user detail, and no change to client-facing responses.
