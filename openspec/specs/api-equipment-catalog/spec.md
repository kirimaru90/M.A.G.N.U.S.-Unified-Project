...
- An `add` or `update` op whose `entry.kind` is not `weapon`, `armor`, `consumable`, or `misc` SHALL be rejected with HTTP 400.
- An `add` or `update` op supplying a tag whose `type` is not `core` or `extra` SHALL be rejected with HTTP 400.
- An `add` or `update` op supplying non-empty `tags` for a `consumable` or `misc` SHALL be rejected with HTTP 400.
...
When the equipment catalog is empty, the system SHALL seed it with the reference design's starter loadouts: the four starter weapon kits and three starter armor kits from the game manual (each with its `core`/`extra` tags and `isStarter: true`), plus a `stimpack` consumable with `isStarter: true` representing the fixed starter dotazione ("Dotazione fissa: Stimpack incluso"; the character receives `quantity: 1` at instantiation, since templates carry no default quantity). This mirrors `api-skills-catalog`'s "Default skills seed the catalog when empty".
