# api-character-notes Specification

## Purpose
A per-character notes collection with full CRUD, scoped and access-controlled like characters. Notes are titled, dated, freeform text living in their own collection (one document per note) and attached to a single character.
