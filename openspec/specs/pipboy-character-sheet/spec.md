...
Character sheet screens for `apps/pip-boy`: read-only S.P.E.C.I.A.L./skills/perks display, an owner/admin-writable action-points stepper, status/conditions editor with catalog quick-pick, inventory/gear editor, resources display/edit, and a client-side (non-persisted) dice roller that adjusts action points per the game rules.
