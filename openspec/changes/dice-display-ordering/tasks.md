...
- [ ] 1.1 In `apps/pip-boy/src/tabs/dice.js` `diceGrid()`, when rendering settled dice
...
- [ ] 1.2 Leave the mid-tumble branch (flickering faces) rendering in place, unsorted.
- [ ] 1.3 Verify `s.selected`, `droppedIndex`, and the reroll `map` continue to operate on original
      indices — no display-order value leaks into them.

## 2. Animate only rerolled dice

- [ ] 2.1 Add an optional `animating` (index `Set`) parameter to `tumble`; a tick re-randomises only
...
- [ ] 2.2 In `reroll()`, capture the selected index set **before** calling `tumble` (which clears
      `s.selected`) and pass it as `animating`. Leave `roll()` calling `tumble` with no `animating`
      set.

## 3. Tests

- [ ] 3.1 Playwright: seed `window.__PB_DICE_RANDOM__` so a roll yields a known unordered pool; assert
      the rendered dice read highest→lowest.
- [ ] 3.2 Playwright: with `SVANTAGGIO` active, assert the dropped (highest) die renders
      struck-through at the front of the sorted row and is not selectable.
- [ ] 3.3 Playwright: select one die and reroll; assert mid-tumble only the selected die's face
      changes while the others hold steady, and the settled outcome resolves over the full pool.
- [ ] 3.4 Run `npx playwright test` from `apps/pip-boy`; confirm the existing dice engine unit tests
