...
**Reason**: This requirement described a single same-origin policy keyed on the `dati/` static-content directory: cache-first for the shell and stale-while-revalidate for `dati/*`. The `dati/` directory has been removed and all content now arrives through the API, where the public/authenticated distinction — not a directory path — determines cacheability. It is replaced by the three new requirements "Three-class request classification", "Authenticated-only responses are never cached", and "Public API content uses a separate stale-while-revalidate cache".

**Migration**: Cache-first behaviour for the shell is preserved by "Three-class request classification" (shell class) and "Service worker pre-caches the application shell". Stale-while-revalidate now applies to public API content (no longer to `dati/*`) per "Public API content uses a separate stale-while-revalidate cache". No client API change is required; the `dati/` directory and its only code reference (`src/screens/boot.js`) are deleted as part of this change.
