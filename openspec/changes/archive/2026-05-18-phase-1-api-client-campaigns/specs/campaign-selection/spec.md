...
On load, the Terminal SHALL show the campaign-selection screen as its first screen. No other screen (boot, terminal-list, login, terminal) SHALL render before the campaign-selection screen has determined how to proceed.

#### Scenario: Fresh load shows campaign selection
- **WHEN** the user loads `index.html` and the DOM is ready
...
The campaign-selection screen SHALL NOT render a `[ Accedi ]` (real-user login) button in this phase. The screen operates in anonymous mode only. The `[ Accedi ]` affordance is introduced in a later phase.

#### Scenario: No login button on campaign selection
- **WHEN** the campaign-selection screen is rendered in any state (empty, single, multi, error)
- **THEN** no button labelled `[ Accedi ]` SHALL be present
