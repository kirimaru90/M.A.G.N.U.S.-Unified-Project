...
- FROM: `### Requirement: Terminals list page is scoped to a campaign`
- TO: `### Requirement: Terminals list page is campaign-agnostic in the route and reads the current campaign`

- FROM: `### Requirement: Sidebar exposes Terminali entry when a campaign is selected`
- TO: `### Requirement: Sidebar exposes an always-enabled Terminali entry`

- FROM: `### Requirement: Create terminal via dialog produces a minimal valid stub`
- TO: `### Requirement: Create/import terminal source the campaign from the current-campaign service`

- FROM: `### Requirement: Terminal detail page shows metadata and editor`
- TO: `### Requirement: Terminal detail page shows the title header, actions, and editor`
...
**Reason**: The terminals list route no longer carries a `:campaignId` parameter, so there is no route id to validate against the cached campaign list. Campaign scope now comes from the in-page selector / `CurrentCampaignService.currentCampaign()`, and the "no campaign" case is handled by the select-a-campaign empty state rather than a not-found guard.
**Migration**: Navigate to `/terminals` and select a campaign in the in-page selector; the list loads for the selected campaign. There is no campaign-id-in-URL to guard, so no existence check is needed.
