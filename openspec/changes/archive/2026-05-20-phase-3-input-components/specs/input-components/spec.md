...
A node MAY declare a `components` array in the holotape JSON. An input component is an entry of that array with `type: "input"`. The entry SHALL declare:
...
- `branches: [<branch>, …]` — an ordered, non-empty list of branches. Each `<branch>` SHALL be either `{ "condition": <Condition>, "target": "<nodeId>" }` or `{ "default": true, "target": "<nodeId>" }`, where `<Condition>` uses the same structured grammar as node `variants` (`and` / `or` / `not` / leaf `{ var, op, value }` with operators `eq` / `neq` / `gt` / `gte` / `lt` / `lte` / `in`).

A node SHALL declare at most one `components` entry with `type: "input"`.

#### Scenario: Node declares an input component
- **WHEN** a node's load payload contains `components: [{ type: 'input', placeholder: 'CODICE', target: 'local.entered_code', branches: [...] }]`
...
- **WHEN** a node's load payload contains no `components` array (or an empty one)
...
On submission (Enter in the field or activation of the submit affordance) with a non-empty value, the engine SHALL dispatch a single mutation `{ op: 'set', key: <target>, value: <rawValue> }` through the same mutation path used by `on_enter` and `choice.set` (scope-routed to `POST /terminals/:id/state/mutate` for `local.*` targets or `POST /campaigns/:id/state/mutate` for `global.*` targets). The submitted value SHALL be sent as the raw field string; the engine SHALL NOT parse or coerce it. On a 2xx response the engine SHALL replace the corresponding scope of the in-memory store from the response's `state` map (per the Phase 2 mutation-response contract).

#### Scenario: Submission posts a set mutation to the correct endpoint
- **WHEN** the user submits the value `58874645` into a component whose `target` is `local.entered_code` on terminal `T1`
...
- **WHEN** the user submits a value into a component whose `target` is `global.codename` in campaign `C1`
...
After a successful (2xx) submission, the engine SHALL evaluate the component's `branches` in declaration order against the snapshot that reflects the just-applied mutation. The first branch whose `condition` evaluates `true` SHALL determine the target node. A branch with `default: true` SHALL act as the fallback when no preceding conditional branch matches. The engine SHALL NOT evaluate branches before the mutation response has been applied to the store.
...
- **WHEN** the user submits `58874645` into a `local.entered_code` target whose first branch is `{ condition: { var: 'local.entered_code', op: 'eq', value: '58874645' }, target: 'code-ok' }`
