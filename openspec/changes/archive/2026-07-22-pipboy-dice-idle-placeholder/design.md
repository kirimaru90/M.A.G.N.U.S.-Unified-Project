...
**Goals:**
- `#pb-dice-result` always occupies its layout slot at full opacity; its content is a pure function of `s.result`, independent of `s.rolling`.
...
**Result box content keyed on `s.result`, not `s.rolling`.**
Gating purely on whether a result exists (rather than additionally requiring `!s.rolling`) means the box's content only changes when `onSettled` reassigns `s.result` — which happens to produce two effects for free: a `-` placeholder through a first-ever roll's tumble (since `s.result` is still `null`), and the *previous* outcome staying visible through a reroll's tumble (since `s.result` isn't cleared until the reroll settles). Both are treated as intended behavior, not edge cases to special-case away — reversing them would mean reintroducing an explicit `s.rolling` check, adding complexity to remove a side effect that's arguably more informative than a blank flash.

Alternative considered: clear `s.result` to `null` when a reroll starts, so the box always reverts to `-` mid-tumble regardless of roll vs. reroll. Rejected — it reintroduces the exact layout-jump/flicker problem this change exists to remove, just scoped to reroll instead of the first roll.

**The box is always rendered at full opacity; a `-` placeholder reserves the space, not `opacity: 0`.**
The box always shows *something* — a dash before the first roll, the outcome after — so there is no empty/hidden state to manage. The element keeps its border and padding in both states so its box height never changes.
...
- **[Risk] Reversing a recently-archived decision may resurface the original complaint** (a static box occupying space before dice exist) → **Mitigation**: the idle box shows a single minimal `-` glyph, not the old wordy `— TIRA I DADI —` text; it reserves the box's space with far less visual weight than the original placeholder carried.
...
- **[Risk] Placeholder dice could be mistaken for a real (favorable) roll at a glance** → **Mitigation**: dimmed styling distinct from `.pb-die--full`'s bright glow, plus the result box showing `-` alongside them instead of an outcome, signals "nothing has happened yet."
