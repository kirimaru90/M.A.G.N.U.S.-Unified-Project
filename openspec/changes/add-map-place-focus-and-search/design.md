...
- **Popup options: `autoPan`, `autoClose`, `closeButton` off; `closeOnClick` on.** `closeOnClick` closes the popup on a genuine tap of the empty map — Leaflet fires that via `preclick`, which it suppresses after a drag (a pan never closes it) and never fires on wheel/pinch zoom; the marker and the popup body stop it too, so only a click *outside* dismisses it. The popup also closes when its marker is torn down (place opened, or panned off — the latter handled explicitly on `moveend` since a pan leaves the marker set untouched), or when another marker's popup opens (Leaflet keeps a single map popup). `autoPan` off so opening it never shifts the centre away from the place `focusPlace` just centred.
