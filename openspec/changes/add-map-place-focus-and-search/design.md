...
- **`zoomSnap: 0` on the `L.map`.** `containZoom`/`revealZoom` are fractional. Leaflet's default `zoomSnap: 1` rounds `setView` to an integer and can round *below* the least-open zoom, leaving the place closed after *Vedi mappa*. Snapping off honours the derived zoom exactly.
- **`containZoom` inward nudge `CONTAIN_EPS = 1e-6` zoom levels.** Guards the `log2`/`2^z` round-trip so `isOpen` is robustly true at the returned zoom rather than landing a float hair outside the circle.
