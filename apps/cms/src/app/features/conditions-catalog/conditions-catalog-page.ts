...
import type {
  ...
  ConditionPolarity,
  ConditionSeverity,
} from '../../core/conditions-catalog/conditions-catalog.types';
          ...
          <ng-template pTemplate="body" let-entry>
            @if (editingSlug() === entry.slug) {
              <tr>
                ...
                <td>
                  <select class="bo-select" [(ngModel)]="draft.defaultSeverity" style="width: 100%;">
                    <option value="minor">minor</option>
                    <option value="major">major</option>
                  </select>
                </td>
                <td>
                  <select class="bo-select" [(ngModel)]="draft.polarity" style="width: 100%;">
                    <option value="positive">positive</option>
                    <option value="negative">negative</option>
                  </select>
                </td>
                <td><input pInputText [(ngModel)]="draft.description" style="width: 100%;" /></td>
                <td>
              ...
              <tr>
                ...
                <td>{{ entry.name }}</td>
                <td>
                  <span class="bo-pill" [class.active]="entry.defaultSeverity === 'major'">
                    {{ entry.defaultSeverity }}
                  </span>
                </td>
                <td>
                  <span class="bo-pill" [class.active]="entry.polarity === 'positive'">
                    {{ entry.polarity }}
                  </span>
                </td>
          ...
          <ng-template pTemplate="footer">
            @if (addRowVisible()) {
              <tr>
                ...
                <td><input pInputText [(ngModel)]="draft.description" placeholder="descrizione" style="width: 100%;" /></td>
                <td>
                  <div style="display: flex; flex-direction: column; gap: 4px; align-items: flex-start;">
                    @if (rowError()) {
                      <small style="color: var(--p-red-500, #ef4444);">{{ rowError() }}</small>
          ...
          <ng-template pTemplate="emptymessage">
            <tr>
              <td colspan="6" style="text-align: center; color: var(--bo-text-faint);">
                Nessuna condizione nel catalogo
              </td>
            </tr>
          </ng-template>
        </p-table>
...
export class ConditionsCatalogPage implements OnInit {
  private readonly api = inject(ConditionsCatalogApiService);
  private readonly confirmationService = inject(ConfirmationService);
  private readonly messageService = inject(MessageService);

  protected readonly entries = signal<ConditionCatalogEntryDto[] | undefined>(undefined);
    ...
    this.api
      .patchSchema([
        {
          ...
          entry: {
            name,
            defaultSeverity: this.draft.defaultSeverity,
            polarity: this.draft.polarity,
      ...
      .subscribe({
        next: () => {
          ...
          this.rowError.set(null);
          this.load();
          this.messageService.add({ severity: 'success', summary: 'Condizione aggiunta' });
        },
        error: (err) => this.rowError.set(this.errorMessage(err)),
      });
  }

  protected startEdit(entry: ConditionCatalogEntryDto): void {
    this.addRowVisible.set(false);
    this.draft = {
      slug: entry.slug,
      name: entry.name,
  ...
  protected submitEdit(entry: ConditionCatalogEntryDto): void {
    ...
    this.api.patchSchema([op]).subscribe({
      next: () => {
        ...
        this.rowError.set(null);
        this.load();
        this.messageService.add({ severity: 'success', summary: 'Condizione aggiornata' });
      },
      error: (err) => this.rowError.set(this.errorMessage(err)),
    });
  }

  protected confirmDelete(entry: ConditionCatalogEntryDto): void {
    this.confirmationService.confirm({
      message: `Eliminare la condizione "${entry.name}"?`,
      acceptLabel: 'Elimina',
      rejectLabel: 'Annulla',
      ...
      accept: () => {
        this.api.patchSchema([{ action: 'delete', slug: entry.slug }]).subscribe({
          next: () => {
            this.load();
            this.messageService.add({ severity: 'success', summary: 'Condizione eliminata' });
          },
          error: () =>
