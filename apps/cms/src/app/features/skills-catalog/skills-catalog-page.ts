...
import type { SkillCatalogEntryDto } from '../../core/skills-catalog/skills-catalog.types';
...
const emptyDraft = (): DraftRow => ({ slug: '', name: '', description: '' });

@Component({
  selector: 'app-skills-catalog-page',
  standalone: true,
...
export class SkillsCatalogPage implements OnInit {
  private readonly api = inject(SkillsCatalogApiService);
  private readonly confirmationService = inject(ConfirmationService);
  private readonly messageService = inject(MessageService);

  protected readonly entries = signal<SkillCatalogEntryDto[] | undefined>(undefined);
  ...
  protected submitAdd(): void {
    ...
    this.api
      .patchSchema([
        { action: 'add', slug, entry: { name, description: this.draft.description.trim() || undefined } },
      ])
      .subscribe({
        next: () => {
          this.addRowVisible.set(false);
          this.rowError.set(null);
          this.load();
          this.messageService.add({ severity: 'success', summary: 'Abilità aggiunta' });
        },
        error: (err) => this.rowError.set(this.errorMessage(err)),
      });
  }

  protected startEdit(entry: SkillCatalogEntryDto): void {
    this.addRowVisible.set(false);
    this.draft = { slug: entry.slug, name: entry.name, description: entry.description ?? '' };
    this.rowError.set(null);
  ...
  protected submitEdit(entry: SkillCatalogEntryDto): void {
    ...
    const op =
      newSlug !== entry.slug
        ...
        : {
            ...
            slug: entry.slug,
            entry: { name, description: this.draft.description.trim() || undefined },
          };
    ...
    this.api.patchSchema([op]).subscribe({
      next: () => {
        ...
        this.rowError.set(null);
        this.load();
        this.messageService.add({ severity: 'success', summary: 'Abilità aggiornata' });
      },
      error: (err) => this.rowError.set(this.errorMessage(err)),
    });
  }

  protected confirmDelete(entry: SkillCatalogEntryDto): void {
    this.confirmationService.confirm({
      message: `Eliminare l'abilità "${entry.name}"?`,
      acceptLabel: 'Elimina',
      rejectLabel: 'Annulla',
      ...
      accept: () => {
        this.api.patchSchema([{ action: 'delete', slug: entry.slug }]).subscribe({
          next: () => {
            this.load();
            this.messageService.add({ severity: 'success', summary: 'Abilità eliminata' });
          },
          error: () =>
